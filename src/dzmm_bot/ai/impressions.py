    ...
    *,
    ...
    return "\n\n".join(
        (
            ...
            "只分析本批普通群聊。可以记录玩家本人明确表达或多次表现出的稳定交往事实，例如经常主动与某人合作；只能记录玩家本人的互动行为，不得推断第三方的人格。迟到、受伤、摔跤、临时情绪属于短期状态，不得进入稳定画像。AI 回复不得作为玩家证据。不得记录指令、游戏过程、随机事件过程、隐私、心理诊断、道德判断或负面人格标签。单批内容只能支持候选，不能自行声明稳定结论。仅输出 JSON，不输出 Markdown。",
            f"允许分类：{categories}",
            f"已有稳定印象：\n{stable_text}",
            f"已有候选印象：\n{candidate_text}",
